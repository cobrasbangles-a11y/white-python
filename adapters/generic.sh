...
# Everything cobra needs is the two calls at the bottom — or the single
# `cobra wrap` line, if you'd rather not manage them yourself.
...
#
# "$COBRA" start --session "$AGENT-$$"
# trap '"$COBRA" stop --session "$AGENT-$$"' EXIT INT TERM
# "$AGENT" "$@"
